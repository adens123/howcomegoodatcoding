...
//將輸入的待辦事項 放到 span裡
    ...
    iconC.className = 'fa-regular fa-heart'
...
function downToggle(e) {
  let iconC = e.target;
  // let iconC = e.target.closest('li').querySelector('i');
  let showFinish = e.target.closest('li').querySelector('.finish');
  
  if (iconC.className.includes('fa-heart')) {
    showFinish.textContent = '[完成]';
    iconC.className = 'fa-solid fa-circle-check'
  } else {
    showFinish.textContent = '[未完成]';
    iconC.className = 'fa-regular fa-heart'
  }
